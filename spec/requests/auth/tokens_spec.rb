...
RSpec.describe "Api::V1::Auth::Tokens", type: :request do
  ...
  describe "POST /api/v1/auth/token/refresh_token" do
    context "with valid refresh token" do
      let(:refresh_token) { create(:refresh_token, user: user) }
      
      it "returns new access token" do
        post "/api/v1/auth/token/refresh_token", params: { refresh_token: refresh_token.token }, as: :json
    ...
    context "with invalid refresh token" do
      it "returns unauthorized for non-existent token" do
        post "/api/v1/auth/token/refresh_token", params: { refresh_token: "invalid-token" }, as: :json
      ...
      it "returns unauthorized for expired token" do
        ...
        post "/api/v1/auth/token/refresh_token", params: { refresh_token: expired_token.token }, as: :json
      ...
      it "returns unauthorized when no token provided" do
        post "/api/v1/auth/token/refresh_token", params: {}, as: :json
  ...
  describe "DELETE /api/v1/auth/token/logout_all" do
    ...
    context "with valid authentication" do
      ...
      it "deletes all refresh tokens for the user" do
        expect {
          delete "/api/v1/auth/token/logout_all", headers: auth_headers, as: :json
      ...
      it "destroys all refresh tokens for the user but not for other users" do
    ...
    context "without authentication" do
      it "returns unauthorized" do
        delete "/api/v1/auth/token/logout_all", as: :json
    ...
    context "with invalid token" do
      it "returns unauthorized" do
        delete "/api/v1/auth/token/logout_all", headers: { 'Authorization' => "Bearer invalid-token" }, as: :json
